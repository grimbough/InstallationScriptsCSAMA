...
To save time, we have performed we above steps already for you and placed the produced files and R objects in a data package called \textit{EpigeneticsCSAMA}, which we load now. (Note that such a data package is used for convenience in this course, but typically, you would not package up interemediate data in this way.) 
