...
%\VignettePackage{EpigeneticsCSAMA}
...
\begin{document}
\title{Basics of ChIP-seq data analysis}
...
H3K27ac is a histone modification associated with active promoters and enhancers. We downloaded data corresponding to a ChIP-seq experiment with two biological replicates of mouse Embryonic Stem cells (mESC) along with the input control sample \textit{Histone H3K27ac separates active from poised enhancers and predicts developmental state} by Creyghton \textit{et al}., PNAS 2010. 
...
Two steps from this vignette (visualization and read distribution analysis) require \textit{biomart} database querying via the internet. In order to avoid many downloads, we provide the necessary objects in the data package \emph{EpigeneticsCSAMA}. The code for the generation of these objects can be found in the \textit{Appendix} of the vignette.
...
The data files produced by the steps above are placed in the R objects of a data package called \textit{EpigeneticsCSAMA}, which we load now. (Note that such a data package is used for convenience in this course, but typically, you would not package up interemediate data in this way.) 
...
The variable \textit{dataDirectory} shows the directory containing the
data objects necessary for this vignette.

<<DirShow>>=
dataDirectory
@

In order to explore this files, have a look at them with a text editor or via 
a terminal emulator.
...
We will tile the genome into non-overlapping bins of size 200 bp. To this end we need the information about chromosome sizes in the mouse genome (assembly \textit{mm9}). In the data package, we provide the object \textit{si} (strand information), which holds these data. The reader can find the code necessary to create the \textit{si} object in the \textit{Obtaining \textit{si} object for \textit{mm9}} of the \textit{Appendix}.
...
In the next section, we see how to visualize bedGraph files using \textit{R}.

\section{Visualisation of ChIP-seq data wit \textit{Gviz}}

Now, we have data which we would like to display along the genome. \textit{R} offers a flexible infrastructure for visualisation of many types of genomics data. Here, we use the \textit{Gviz} package for these purposes.
...
We next add our two data tracks to the plot. We first generate \textit{DataTrack} objects with \textit{DataTrack} function. We include the information about how the track is be labaled and colored. We obtain \textit{input.track}, \textit{rep1.track} and \textit{rep2.track} objects. 
...
If a peak in one replicate overlaps with mutiple peaks in the other replicate, it will appear multiple times in \textit{ovlp}. To see, how many peaks overlap with something in the other replicate, we count the number of unique peaks in each of the two columns of \textit{ovlp} and take the smaller of these two counts to as the number of common peaks.
